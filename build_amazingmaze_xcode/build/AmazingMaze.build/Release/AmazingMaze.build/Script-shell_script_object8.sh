#!/bin/bash
set -e
if [ "$CONFIGURATION" = "Debug" -o "$CONFIGURATION" = "Debug ARM" -o "$CONFIGURATION" = "Debug AARCH64" ]; then echo "skipping for debug builds" && exit 0; fi
if [ "$CONFIGURATION" = "Debug" -o "$CONFIGURATION" = "Release" -o "$CONFIGURATION" = "Debug ARM" -o "$CONFIGURATION" = "Release ARM" ]; then echo "skipping for x86 and arm builds" && exit 0; fi
...
s3e_plink -o "/Users/xiaolilillytang/Desktop/AmazingMaze/build_amazingmaze_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/AmazingMaze.s3e" --arch=armv8-a --compiler=gcc -u --usemap="/Users/xiaolilillytang/Desktop/AmazingMaze/build_amazingmaze_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/AmazingMaze.map" "${TARGET_BUILD_DIR}/${EXECUTABLE_NAME}"
